...
返回 Cloudflare Cron 主动探测产生的最近 24 小时平台 Uptime，包括当前状态、测试歌曲可播率、平均耗时和逐小时状态条。GDStudio 返回官方平台 CDN 且报告文件大于 1 MB 时采用可信元数据验证，避免 Cron 机房跨地域探测网易云 CDN 造成误判；其他结果继续执行 Range 音频验证。公开数据只包含 netease/qq/kuwo 平台汇总，不包含内部解析源。

Cron 每 5 分钟运行一次并轮换平台，因此每个平台约 15 分钟完成一次主动检测。每轮先检测一首轮换样本；失败时最多再选择两首不同样本复核，任一首通过“解析链接 + 音频有效性验证”即判本轮平台可用，三首都失败才判失败。QQ 使用六首不同歌手和版权类型的样本池；版权限制会单独记为 `copyright_unavailable`。探测结果写入独立的 `service_uptime_checks` 表，不与管理员页面中的用户真实调用统计混合。
...
读取指定用户的脱敏 API Key、最近使用、绑定设备、启用状态和 `network_risk` 网络活动摘要。摘要只包含脱敏网络段、Cloudflare 地区/ASN、最近时间和异常原因，不保存或返回完整公网 IP，也不会返回完整 Key 或设备令牌。
...
Worker 会缓存短期链接、合并同一歌曲的并发解析并按健康度排序内部来源。缓存未命中时，默认先并发请求 3 个健康度最高的来源；400ms 内没有得到经过验证的音频，就立即放出其余全部可用来源。任一来源成功后会取消其余竞速请求；只有全部可用来源失败或达到整体超时才返回失败。首批数量和扩展延迟可通过 `RESOLVER_INITIAL_CONCURRENCY`、`RESOLVER_EXPANSION_DELAY_MS` 调整。失败结果默认短暂缓存 20 秒，避免多人重复请求同一首受限歌曲时持续打满上游；时长可通过 `RESOLVER_NEGATIVE_CACHE_TTL_SECONDS` 调整。为避免暴露内部服务链路，对外仅返回 `data.url`、`cover` 与 `lyrics`。
...
通过鉴权后，Worker 会异步记录该 Key 的脱敏访问网络。IPv4 按 `/24`、IPv6 按 `/64` 聚合并加盐哈希，只用于会员管理后台提醒异常使用；网络变化不会自动阻止播放。
...
- 各平台同时搜索并解析，第一个通过音频验证的平台结果胜出并取消其余任务；平台内按标题和歌手匹配度优先尝试候选，整体预算 5.5 秒。解析阶段与网页播放、Cron 共用两批智能竞速和音质回落规则。

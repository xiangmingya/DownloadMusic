...
const authenticatedUserId = String(APP_CONTEXT?.user?.linuxdo_id || APP_CONTEXT?.user?.bimoji_sub || APP_CONTEXT?.user?.id || '').trim();
const playModeStorageKey = `${LOCAL_KEY_PREFIX}playmode_${AUTH_TYPE}_${authenticatedUserId || 'default'}`;
const playlistStorageKey = `${LOCAL_KEY_PREFIX}playlist_${AUTH_TYPE}_${authenticatedUserId || 'default'}`;
const favoriteStorageKey = `${LOCAL_KEY_PREFIX}favorites_${AUTH_TYPE}_${authenticatedUserId || 'default'}`;
const recentStorageKey = `${LOCAL_KEY_PREFIX}recent_${AUTH_TYPE}_${authenticatedUserId || 'default'}`;
const savedPlaylistStorageKey = `${LOCAL_KEY_PREFIX}saved_playlists_${AUTH_TYPE}_${authenticatedUserId || 'default'}`;
...
function initStaticIcons() {
    ...
    document.querySelectorAll('.uptime-dialog-close').forEach(button => setButtonIcon(button, 'close', 18));
...
// 所有新播放、下载和预加载统一交给 Worker 解析器；Worker 负责缓存、竞速和来源选择。
async function resolveSongMedia(song, quality, options = {}) {
    const platform = toPrimaryPlatform(song?.platform || song?.source);
    ...
    if (!response.ok || Number(payload?.code) !== 0 || !payload?.data?.url) {
        throw new Error(getApiErrorMessage(payload, response.status, '未获取到播放链接'));
    }
    ...
    const parsed = {
        ...
        error: '',
        pic: normalizeMediaUrl(data.cover || song?.cover || ''),
        cover: normalizeMediaUrl(data.cover || song?.cover || ''),
        lyrics: String(data.lyrics || ''),
        info: {
            name: String(song?.name || `ID ${id}`),
...
async function resolvePreloadMediaUrl(song, quality) {
    ...
    if (!item?.id) return '';
    const parsed = await resolveSongMedia(item, quality).catch(() => null);
...
async function searchSongsByKeywordPagePrimary(keyword, selectedPlatform, options = {}) {
    ...
    const url = new URL(API_ROUTES.search, window.location.href);
    url.searchParams.set('platform', platform);
    ...
    if (!response.ok || Number(payload?.code) !== 0) {
        throw new Error(getApiErrorMessage(payload, response.status, '搜索失败'));
    }
...
async function search() {
    ...
    try {
        ...
        if (currentSearchType === 'song') {
            const resolved = await resolveSongMedia({
                platform,
                id: input,
                name: `ID ${input}`,
                artist: '未知歌手'
            ...
            if (resolved) {
                displaySongsWithPagination([toSongFromParsedItem(platform, resolved)]);
            } else {
                resultsDiv.innerHTML = '<div class="empty-state">解析失败，请稍后重试或切换平台</div>';
            }
        } else {
    ...
    resultsDiv.innerHTML = pageSongs.map((song, index) => {
        ...
        <div class="result-item" id="song-${globalIndex}">
            <div class="song-header">
                ...
                <div>
                    <button class="play-btn-item" data-index="${globalIndex}" data-play-key="${escapeHtml(platform)}:${escapeHtml(song.id)}" onclick="playSong('${safePlatform}', '${safeId}', '${safeName}', '${safeArtist}', ${globalIndex})">${getIconSvg('play', 20)}</button>
...
async function resolveDownloadMedia(source, id, name, artist, runtimeSong, quality, options = {}) {
    const parsed = await resolveSongMedia(runtimeSong || { source, id, name, artist }, quality, options);
...
async function resolveFreshPlaybackUrl(platform, id, name, artist, quality, runtimeSong) {
    const parsed = await resolveSongMedia(runtimeSong || { platform, id, name, artist }, quality, { bypassCache: true }).catch(() => null);
    return normalizeMediaUrl(parsed?.url || '');
}
...
function syncInlinePlayButtonState() {
    ...
    document.querySelectorAll('[data-play-collection]').forEach(btn => {
        btn.innerHTML = getIconSvg('play', 32);
    });
    document.querySelectorAll('[data-play-saved-song]').forEach(btn => {
        btn.innerHTML = getIconSvg('play', 20);
    });
    document.querySelectorAll('[data-play-sheet]').forEach(btn => {
        btn.innerHTML = '播放';
    });
    if (currentPlayKey && currentPlayingSong && !audio.paused) {
        playButtonsByKey(currentPlayKey).forEach(btn => {
            if (btn.classList.contains('play-btn-item')) btn.innerHTML = getIconSvg('pause', 16);
            else if (btn.hasAttribute('data-play-saved-song')) btn.innerHTML = getIconSvg('pause', 20);
            else if (btn.hasAttribute('data-play-sheet')) btn.innerHTML = '播放中';
            else btn.innerHTML = getIconSvg('pause', 32);
        });
    }
    // 兼容 lookupOnly 榜单：按当前序号回退显示暂停图标。
...
function currentPlayKeyOf(song) {
    return `${String(song?.platform || song?.source || '')}:${String(song?.id || '')}`;
}

function playButtonsByKey(key) {
    if (!key) return [];
    try {
        return Array.from(document.querySelectorAll(`[data-play-key="${CSS.escape(key)}"]`));
    } catch {
        return [];
    }
}

function setPlayButtonLoading(key, loading) {
    playButtonsByKey(key).forEach(btn => {
...
function songRowMarkup(song, collection, index, allowFavoriteRemove = false) {
    ...
    const favoriteControl = allowFavoriteRemove
        ...
        : '';
    return `<div class="mini-song-row${allowFavoriteRemove ? ' has-favorite-control' : ''}">${image}<div class="mini-song-meta"><strong>${escapeHtml(song.name)}</strong><small>${escapeHtml(song.artist)}</small></div><button type="button" data-play-collection="${collection}" data-song-index="${index}" data-play-key="${escapeHtml(song.platform || song.source || '')}:${escapeHtml(song.id || '')}" aria-label="播放 ${escapeHtml(song.name)}">${getIconSvg('play', 32)}</button>${favoriteControl}</div>`;
}
    ...
    const songRows = playlist.songs.length
        ? playlist.songs.map((song, index) => {
            ...
            const art = cover
                ? `<img src="${escapeHtml(cover)}" alt="" onerror="this.outerHTML='<span class=mini-song-art></span>'">`
                : '<span class="mini-song-art" aria-hidden="true"></span>';
            return `<div class="saved-playlist-song">${art}<div class="mini-song-meta"><strong>${escapeHtml(song.name)}</strong><small>${escapeHtml(song.artist)}</small></div><button type="button" class="saved-playlist-song-play" data-play-saved-song="${escapeHtml(playlist.id)}" data-song-index="${index}" data-play-key="${escapeHtml(song.platform || song.source || '')}:${escapeHtml(song.id || '')}" aria-label="播放 ${escapeHtml(song.name)}">${getIconSvg('play', 20)}</button><button type="button" class="saved-playlist-song-remove" data-remove-saved-song="${escapeHtml(playlist.id)}" data-song-index="${index}" aria-label="从歌单移除 ${escapeHtml(song.name)}">${getIconSvg('close', 18)}</button></div>`;
        }).join('')
...
async function loadApiKeyStatus() {
    ...
    try {
        ...
        resetLink.hidden = data.enabled === false;
...
function renderAdminApiKeyDetails(data) {
    ...
    const key = data.key;
    const device = key.device_name || key.device_id || '尚未绑定';
    const risk = key.network_risk || {};
    const attention = risk.available !== false && risk.status === 'attention';
    const reasons = Array.isArray(risk.reasons) ? risk.reasons : [];
    const networks = Array.isArray(risk.recent_networks) ? risk.recent_networks : [];
    const riskPanel = risk.available === false
        ? '<section class="admin-api-network-panel is-unavailable"><strong>网络活动尚不可用</strong><p>请先执行最新的 D1 数据库迁移。</p></section>'
        : `<section class="admin-api-network-panel${attention ? ' is-attention' : ''}"><div class="admin-api-network-heading"><div><small>API 使用检测</small><strong>${attention ? '发现异常，需人工确认' : '未发现明显异常'}</strong></div><span>${attention ? '需关注' : '正常'}</span></div>${reasons.length ? `<ul>${reasons.map(reason => `<li>${escapeHtml(reason)}</li>`).join('')}</ul>` : ''}<div class="admin-api-network-summary"><span>24 小时 ${Number(risk.networks_24h || 0)} 个网络段</span><span>7 天 ${Number(risk.networks_7d || 0)} 个网络段</span></div>${networks.length ? `<div class="admin-api-network-list"><h3>最近访问网络</h3>${networks.map(network => {
            const location = [network.country, network.region, network.city].filter(Boolean).join(' · ') || '地区未知';
            const asn = network.asn ? `AS${network.asn}` : '运营商未知';
            return `<div><code>${escapeHtml(network.ip_preview || '未知网络')}</code><span>${escapeHtml(location)} · ${escapeHtml(asn)}</span><time>${escapeHtml(memberTime(network.last_seen_at))}</time></div>`;
        }).join('')}</div>` : '<p class="admin-api-network-empty">等待该 Key 的首次外部接口调用。</p>'}</section>`;
    details.innerHTML = `<div class="admin-api-key-status-row"><code>${escapeHtml(key.key_preview || '—')}</code><span class="monitoring-health ${key.enabled ? 'monitoring-health-healthy' : 'monitoring-health-down'}">${key.enabled ? '已启用' : '已禁用'}</span></div>${riskPanel}<dl><div><dt>最近使用</dt><dd>${escapeHtml(memberTime(key.last_used_at))}</dd></div><div><dt>申请时间</dt><dd>${escapeHtml(memberTime(key.created_at))}</dd></div><div><dt>绑定设备</dt><dd>${escapeHtml(device)}</dd></div><div><dt>设备标识</dt><dd>${escapeHtml(key.device_id || '—')}</dd></div><div><dt>小米账号</dt><dd>${escapeHtml(key.mi_uid_tail || '—')}</dd></div><div><dt>绑定时间</dt><dd>${escapeHtml(memberTime(key.bound_at))}</dd></div></dl>`;
    toggle.hidden = false;
    toggle.dataset.enabled = key.enabled ? '1' : '0';
    toggle.textContent = key.enabled ? '禁用 API Key' : '启用 API Key';
...
async function toggleAdminApiKey() {
    const button = document.getElementById('adminApiKeyToggleBtn');
    if (!button || !activeAdminApiKeyLinuxdoId) return;
...
async function playSongCore(source, id, name, artist, options = {}) {
    ...
    const playKey = String(options.playKey || currentPlayKeyOf({ platform: source, id }));
    ...
    clearTimeout(nextTrackPreloadTimer);
    setPlayButtonLoading(playKey, true);
    setGlobalPlayLoading(true);

    try {
        const resolveInput = runtimeSong || { platform: source, id, name, artist, cover: options.cover || '' };
        const parsed = await resolveSongMedia(resolveInput, quality);
        if (playRequestId !== activePlayRequestId) return;
        let mediaUrl = normalizeMediaUrl(parsed?.url || '');
        if (!mediaUrl) throw new Error('未获取到播放链接');
        const rawCover = normalizeMediaUrl(parsed.cover || parsed.pic || parsed?.info?.pic || options.cover || '');
        const lyricsRaw = String(parsed.lyrics || '');
        const songPlatform = String(resolveInput.platform || resolveInput.source || source);
        const backupMeta = null;
        const backup3Meta = null;
        ...
        setPlayButtonLoading(playKey, false);
        if (playRequestId === activePlayRequestId) setGlobalPlayLoading(false);
    }
}

// 播放歌曲
async function playSong(source, id, name, artist, index, options = {}) {
    const playKey = currentPlayKeyOf({ platform: source, id });
        ...
        return `
            <div class="playlist-item${active}" data-index="${index}">
                ...
                <div class="playlist-item-actions">
                    <button type="button" data-action="play" data-index="${index}" data-play-sheet="1" data-play-key="${escapeHtml(song.platform || song.source || '')}:${escapeHtml(song.id || '')}">播放</button>
